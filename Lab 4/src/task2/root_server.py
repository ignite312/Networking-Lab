...
def handle_dns_request(data, client_address, server_socket):
    ...
    with open('root_dns_records.txt', 'r') as file:
        ...
        if li <= 1:
            ...
            print(name)
            if name[-1] == 'com':
                print('\nNo match in the server. Sending client a Referral to {}:{}'.format(*com_tld_server))
                server_socket.sendto('contact localhost 8010'.encode('utf-8'), client_address)
            elif name[-1] == 'bd':
                print('\nNo match in the server. Sending client a Referral to {}:{}'.format(*bd_tld_server))
                server_socket.sendto('contact localhost 8011'.encode('utf-8'), client_address)
        else:
