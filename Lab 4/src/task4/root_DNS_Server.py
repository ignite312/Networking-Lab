    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as server_socket:
        ...
        while True:
            ...
            if response:
                ...
                print(parts)
                if parts[-1] == 'com':
                    server_socket.sendto(query_name.encode('utf-8'), ('localhost', 8002))
                    response, _ = server_socket.recvfrom(1024)
                    server_socket.sendto(response, addr)
                elif parts[-1] == 'bd':
                    server_socket.sendto(query_name.encode('utf-8'), ('localhost', 8003))
                    response, _ = server_socket.recvfrom(1024)
                    server_socket.sendto(response, addr)
                elif parts[-1] == '' and parts[-2] == 'com':
                    server_socket.sendto(query_name.encode('utf-8'), ('localhost', 8002))
                    response, _ = server_socket.recvfrom(1024)
                    server_socket.sendto(response, addr)
                elif parts[-1] == '' and parts[-2] == 'bd':
                    server_socket.sendto(query_name.encode('utf-8'), ('localhost', 8003))
                    response, _ = server_socket.recvfrom(1024)
                    server_socket.sendto(response, addr)
                else:
...
if __name__ == "__main__":
    DNS_RECORDS_FILE = 'root_server.txt'
