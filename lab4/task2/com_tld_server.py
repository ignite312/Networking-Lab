...
com_auth_server = ('localhost', 8015)
...
def handle_dns_request(data, client_address, server_socket):
    ...
    with open('com_tld_records.txt', 'r') as file:
        li = 0
        for line in file:
            if li == 0:
                li += 1
            ...
            if parts[0] == query_name:
                if parts[2] == "A":
                    ...
                    li += 1
        if li <= 1:
            print('\nNo match in the server. Sending client a Referral to {}:{}'.format(*com_auth_server))
            server_socket.sendto(b'contact localhost 8015', client_address)
        else:
