...
def handle_dns_request(data, client_address, server_socket):
    ...
    with open('bd_tld_records.txt', 'r') as file:
        li = 0
        for line in file:
            if li == 0:
                li += 1
            ...
            if parts[0] == query_name:
                if parts[2] == "A":
                    ...
                    li += 1
        if li <= 1:
            print('\nCould not find match. Forwarding to bd_auth')
            server_socket.sendto(data, bd_auth_server)
            resp, _ = server_socket.recvfrom(4096)
            print('Received response from bd_auth. Backtracking to root')
