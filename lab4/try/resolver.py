...
def handle_dns_request(data, client_address, server_socket):
    ...
    with open('root_dns_records.txt', 'r') as file:
        ...
        if li <= 1:
            ...
            server_socket.sendto(data, (root_server))
            resp, address = server_socket.recvfrom(4096)
            print(f'Received response from {address}. Backtracking to client')
